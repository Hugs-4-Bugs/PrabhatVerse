...
function parseProfile(profile: string): ProfileSections {
  ...
  for (const line of profile.split(/\r?\n/)) {
    ...
    else if (sec.startsWith("## certifications")) cur = "certifications";
    else if (sec.startsWith("## trading concepts")) cur = "trading";
...
export default function ChatbotSection({ messages, setMessages }: {
  messages: Message[];
  setMessages: React.Dispatch<React.SetStateAction<Message[]>>; // Correctly type the setMessages prop
}) {
